...
exports.getFiles = async function(userId, type, page) {
    const count = await File.countDocuments({type: type, userId: userId});
    let limit = page * ITEMS_PER_PAGE;
    ...
    return new Promise(function(resolve, reject) {
        ...
        function(error, files) {
            if (error) {
                console.log("[file model getFiles] ", error);
                reject(error);
            }
            if (files) {
            ...
            resolve({files, showLoadMoreButton: count > limit});
        })
    })
};
...
exports.getFileById = function(fileId) {
    return new Promise(function(resolve, reject) {
        File.findById(fileId)
            ...
            .catch(error => {
                if (error) {
                    console.log("[file model getFileById] ", error);
                    reject(error);
                }
            });
    });
...
exports.deleteFile = function(userId, fileId) {
    return new Promise(function(resolve, reject) {
        ...
        function(error, file) {
            if (error) {
                console.log("[file model deleteFile] ", error);
                reject(error);
            }
            resolve({"success" : 1});
        })
...
exports.saveFile = function(fileData) {
    return new Promise(function(resolve, reject) {
        ...
        file.save((error, file) => {
            if (error) {
                console.log("[file model saveFile] ", error);
                reject(error);
            }
            resolve(file);
        });
