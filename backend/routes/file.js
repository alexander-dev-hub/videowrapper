...
const getFileType = req => {
    ...
    if (utils.isSet(urlParts.query.type)) {
...
const isPublic = req => {
    const urlParts = url.parse(req.url, true);
    if (utils.isSet(urlParts.query.public)) {
        return urlParts.query.public == config.uploadAccessType.public;
    }
...
exports.getFiles = function(req, res) {
    ...
    filesDB.getFiles(userId, getFileType(req), page)
        .then(files => {
            return res.status(200).send(files);
        })
        .catch(error => {
