...
import { screenSize, measureScreenSize, getSpacingUnit, elementSizePosSet, showToastError } from '../../utils/utility';
...
class VideoStudioStepNav extends Component {
    ...
    nextHandler = () => {
        const { videoStepperState, activeStep } = this.props;
    ...
    resetHandler = () => {
        const { updateActiveStep, clearData } = this.props;
        clearData();
        updateActiveStep(0);
    };
    ...
    renderErrorCallback = error => {
        console.log('ray : [components VideoStudioStepNav renderErrorCallback] error => ', error);
        const { setLoadingStatus } = this.props;
        showToastError('Rendering Failed!');
    ...
    alertClosehandler = () => {
        ...
        const { activeStep, editPreviewSwitched } = this.props;
        if (activeStep === steps.length - 1) {
            this.resetHandler();
        } else {
    ...
    render () {
        const { classes, activeStep, width } = this.props;
