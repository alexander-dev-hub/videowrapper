...
import { screenSize, measureScreenSize, showToastInfo, showToastError } from '../../utils/utility';
    ...
    const onUploadHandler = files => {
        ...
        axios.post(uploadUrl, data, {
            onUploadProgress: ProgressEvent => {
                ...
                console.log('ray : [percent] % => ', ProgressEvent.loaded / ProgressEvent.total * 100);
            }
        })
        .then(response => {
            ...
            if (switchToPreview){
                ...
                setUploading(false);
                props.setLoadingStatus({loading: false});
                switchToPreview();
            } else {
                showToastInfo('Uploading Completed!');
                setFiles([]);
                setUploading(false);
                props.setLoadingStatus({loading: false});
            }
        ...
        .catch(error => {
            ...
            showToastError('Uploading Failed!');
        });
    };
...
export default connect(null, mapActionToProps)(withWidth()(DropzoneArea));
