...
import { getDeleteUrl, deleteFile } from '../../utils/api';
...
const VideoGrid = ({
    ...
    const onDeleteHandler = tile => {
        if (deleteHandler) {
            let deleteFileUrl;
            if (publicDelete) {
                deleteFileUrl = getDeleteUrl(tile._id, publicDelete);
            } else {
                deleteFileUrl = getDeleteUrl(tile._id);
            }
    ...
    return (
        <PaperLayer>
            <div className={classes.root}>
                ...
                { showLoadMoreButton && (
                    <LoadMoreFilesButton incrementPageNumber={incrementPageNumber}/>
                ) }
            </div>
        </PaperLayer>
    );
