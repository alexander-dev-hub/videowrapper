...
const getUploadedUrl = (type, page, accessType=uploadAccessType.private) => {
    const uploadedUrl = urlJoin(config.proxyUrl, filesPrefix, `?${uploadTypePrefix}${type}`, `&${pagePrefix}${page}`, `&${UploadAccessTypePrefix}${accessType}`);
    return uploadedUrl;
};
...
const getDeleteUrl = (_id, accessType=uploadAccessType.private) => {
    const deleteUrl = urlJoin(config.proxyUrl, filesPrefix, _id, `?${UploadAccessTypePrefix}${accessType}`);
    return deleteUrl;
};
...
const render1 = (videoStepperState, successCallback, errorCallback) => {
    const { backgroundSize } = videoStepperState;
    const body = JSON.stringify({
        // static values are set
        'background': 0, // image background(1) or color background(0)
